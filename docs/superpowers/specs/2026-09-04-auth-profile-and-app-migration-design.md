...
| `daily_goal_minutes` | 5 / 10 / 15 / 20 | 홈의 "오늘의 목표" 진행바 기준 (§13 참고 — 목표 방식은 재검토 예정) |
...
| 4권 | 22~28 | 복잡한 모음 ㅐ ㅔ ㅟ / ㅘ·ㅢ / ㅚ·ㅙ / ㅝ·ㅞ / ㅒ·ㅖ |
| 5권 | 29~35 | 쌍자음 ㄲ ㄸ ㅃ ㅆ ㅉ, 필기 순서 2단계 |

35단계 전체와 각 단계의 예시 낱말은 사용자가 제공한 워크북 뒷표지 사진에서 확인해
`packages/utils/src/hangul-stages.ts`에 옮겨두었다. 그 파일이 커리큘럼의 단일 원본이며,
자모 게이팅과 낱말·문장 콘텐츠 모두 여기서 파생된다.

책에는 각 권마다 3단계·7단계 뒤의 **복습**과 권말 **정리 학습**도 있다. 앱에는 아직 넣지
않았으나, 반복 학습 지점으로 쓸 수 있다.
...
1단계(모음)에는 읽을 문장이 없다. 그래서 글자를 익히는 단계와 읽는 단계를 나눠
세 활동으로 구성한다. 첫 활동은 아직 "읽기"가 아니라 자음·모음을 익히는 단계라
이름도 `자음모음 배우기`로 두었다.
...
`worksheet`. 직전 읽기 세션의 내용으로 인쇄용 학습지를 만든다. 한 쪽에 5개, 첫 쪽에만
제목과 이름 칸, 각 항목 아래 쓰기 줄. 기존 `@media print` 레이아웃을 그대로 옮긴다.

**쓰기 학습지의 내용은 한글 단계에 따라 달라진다.** 처음 설계는 문장만 상정해
`subject_level 14`(문장 읽기와 같은 시작점)로 두었는데, 그러면 4~13단계 아이는 낱말은
읽으면서 쓸 것이 하나도 없는 구간에 갇힌다. 사용자가 이 구멍을 발견해 아래처럼 바꾸기로 했다.

| 단계 | 학습지에 실리는 것 | 나오는 곳 |
|---|---|---|
| 1~3 | 자음·모음 따라쓰기 (ㅏ ㅑ ㅓ ㅕ / ㄱ ㄴ ㄷ) | 자음모음 배우기 세션 |
| 4~13 | 낱말 (고기, 나비) | 낱말 읽기 세션 |
| 14~ | 문장 (꽃이 예뻐요) | 문장 읽기 세션 |

따라서 이 레슨의 `subject_level`은 **14가 아니라 1**이어야 한다 — Phase 3에서 이 활동을
구현할 때 seed 를 함께 고친다. "직전에 읽은 것을 그대로 쓴다"는 흐름은 세 구간 모두에서
유지된다. 읽기 세션이 아직 없으면 지금 단계에서 새로 뽑아 만든다.
...
## 13. 미결 사항 — "오늘의 목표"를 시간이 아니라 할 일로

Phase 1은 하루 목표를 **분 단위 시간**(`daily_goal_minutes`)으로 두고 홈에 진행바를
띄운다. 활동이 아직 없어 진행바는 늘 0분이고, 활동이 생기면 세션의 `duration_sec`
합계로 채워질 예정이다. 그런데 실제 사용을 앞두고 사용자가 이 방식의 허점을 지적했다.

> 목표가 20분인데 활동이 5분씩이라 10분에 다 끝냈다. 오늘 공부는 끝인가,
> 10분을 더 해야 하나? 이게 사용 제한시간처럼 보이지는 않나?

시간 목표는 **"얼마나 배웠는가"가 아니라 "얼마나 앉아 있었는가"**를 잰다. 그래서
빨리 끝낸 아이는 더 하라는 요구를 받고, 늑장 부린 아이는 앉아 있었다는 이유로
목표를 달성한다. 이 앱에는 문제가 하나 더 있다 — **쓰기와 계산은 종이에서 일어나고
앱은 그 시간을 잴 수 없다**(§0). 시간을 목표로 삼으면 종이 학습이 통계에서 빠진다.

### 합의한 방향 (Phase 2에서 구현 여부 재검토)

목표를 **할 일 개수**로 바꾸고 시간은 **기록**으로만 남긴다.

- 홈 상단: `오늘의 할 일 ●●○ 2 / 3` — 다 하면 그날은 끝이고 축하 화면을 띄운다.
- 그 아래 작게: `오늘 12분 공부했어요` — 채워야 할 막대가 아니라 참고 수치.
- 목표를 채운 뒤에도 더 할 수 있다. 상한이 아니라 완료 표시다.
- 인쇄해서 종이로 한 활동(쓰기 학습지, 100칸 종이 풀이)도 **한 개로 센다.**
- 부모 설정에서 아이별 하루 개수를 정한다.

**지금 바꾸지 않는 이유:** 활동이 하나도 없는 상태에서 목표 형태를 정하면 탁상공론이
된다. Phase 2에서 활동 두세 개를 실제로 만들어 한 판이 몇 분 걸리는지 본 뒤에 정한다.
그때까지 `daily_goal_minutes`와 진행바는 그대로 둔다.

---

## 14. 문제 풀이 활동의 3단계 흐름

정답이 있는 활동(`choice_quiz` — 더하기 놀이, 맞춤법 탐험대)은 모두 같은 흐름을 쓴다.
사용자가 제안한 방식이며, 라윤이 맞춤법 앱에 이미 있던 "문제 풀기 → 오답 복습" 2단계를
3단계로 확장한 것이다.

```
1차   문제 10개를 쭉 푼다 (정답 여부를 그때그때 알려주지 않는다)
      ↓
채점   "10개 중 7개 맞았어요"
      ↓
2차   틀린 3개만 다시 푼다 (역시 채점은 끝나고)
      ↓
채점   "3개 중 2개 맞았어요"
      ↓
3차   남은 1개에 💡힌트를 띄우고, 맞힐 때까지 다시 푼다
      ↓
끝     "7개 → 9개 → 10개" 로 나아진 과정을 보여준다
```

### 왜 이렇게 하나

- **흐름이 끊기지 않는다.** 문제마다 ⭕❌가 뜨면 열 문제를 쭉 푸는 집중이 깨지고,
  채점 순간의 의미도 사라진다.
- **틀린 것만 다시 본다.** 이미 아는 문제를 또 풀리지 않는다.
- **결국 이해하고 끝난다.** 3차에는 힌트가 있으므로 좌절하지 않고 마무리된다.
  맞힐 때까지 남아 있고, 정답을 대신 알려주고 넘어가지 않는다.
...
| 정답 즉시 표시 | ❌ | ❌ | ✅ |
| 힌트 | ❌ | ❌ | ✅ |
| 틀리면 | 다음 문제로 | 다음 문제로 | 같은 문제에 머문다 |

### 점수

**`sessions.correct_count` 에는 1차 정답 개수만 넣는다.** 2·3차에 맞힌 것은 "결국
이해했다"는 뜻이지 실력 수치가 아니다. 라운드별 성적은 `sessions.meta.roundScores`
배열에 남겨 나아진 과정을 보여주는 데 쓴다.

이 규칙은 기존 시윤이 앱의 버그를 구조적으로 해결한다 — 원본은 정답을 맞혀야만 다음
문제로 넘어가서 완료 화면의 점수가 늘 만점이었다.

### 힌트는 어디서 오나

- **맞춤법 탐험대**: 원본 데이터에 보기마다 해설이 이미 들어 있다
  (`"'않다'는 '아니하다'가 줄어든 말이에요"`). 그대로 3차 힌트로 쓴다.
- **더하기 놀이**: 문제에서 만들어낸다 — 그림을 한 줄로 합쳐 세어 보이고,
  `"3에서 시작해서 4, 5 — 이렇게 2만큼 더 세어요"` 처럼 이어세기를 알려준다.

### 정답이 없는 활동은 해당 없음

읽기 활동(`letter_cards` / `word_cards` / `reading_cards`)은 채점 대상이 아니므로 이
흐름을 쓰지 않는다. 읽고 넘기며, 막히는 글자를 톡 누르면 🔊 로 알려주는 방식 그대로다(§7).
