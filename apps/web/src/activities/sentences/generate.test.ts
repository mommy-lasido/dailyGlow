...
describe('자료', () => {
  it('모든 문장이 책의 자모만으로 이루어져 있다', () => {
    for (const s of SENTENCE_ITEMS) {
      expect(minStageFor(s)).not.toBeNull();
    }
  });

  it('같은 문장이 두 번 들어가지 않는다', () => {
    expect(new Set(SENTENCE_ITEMS).size).toBe(SENTENCE_ITEMS.length);
  });
    ...
    const pairs = SENTENCE_ITEMS.filter((a) =>
      SENTENCE_ITEMS.some(
        (b) =>
          ...
          sharedWords(a, b) === a.split(' ').length - 1,
      ),
    );
  ...
  it('배운 데까지의 문장만 고른다', () => {
    for (const stage of [9, 14, 21, 28, 35]) {
      for (const s of poolForStage(stage)) {
        expect(minStageFor(s)!).toBeLessThanOrEqual(stage);
      }
    }
  });
...
describe('pickDistractors', () => {
  ...
  it('같은 낱말을 많이 쓰는 문장을 먼저 고른다', () => {
    ...
    const pool = poolForStage(25);
    const answer = pool.find((s) => s === '곰이 밥을 먹어요')!;
    for (let i = 0; i < 50; i += 1) {
      const wrong = pickDistractors(answer, pool);
      for (const w of wrong) {
        expect(sharedWords(answer, w)).toBeGreaterThan(0);
      }
    }
  });
});
...
describe('makeSentenceSet', () => {
  it('한 판 안에서 같은 문장이 두 번 나오지 않는다', () => {
    ...
    for (let i = 0; i < 100; i += 1) {
      const shown = makeSentenceSet(pool).map((p) => p.answer);
...
describe('sentenceQuestion', () => {
  it('소리를 듣고 찾는 것이라 무엇을 찾는지만 묻는다', () => {
    expect(sentenceQuestion()).toBe('어느 문장일까?');
  });
});
