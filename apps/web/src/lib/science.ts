 ...
 * 영어 영상에서 고를 수 있는 자막.
 *
 * **한국어 영상에는 자막을 두지 않는다.** 우리말은 듣고 아는 것이지 읽고 아는
 * 것이 아니라, 자막이 있으면 글자만 좇게 되고 단추 하나가 더 늘 뿐이다.
 *
 * 영어 영상에는 **아이를 가리지 않고 둘 다 둔다.** 라윤이는 자막 없이
 * 알아들으니 안 켜면 그만이고, 시윤이는 골라 켜면 된다. 아이마다 따로 맞춰
 * 두면 아이가 늘 때마다 손이 가고, 영숙님이 그것까지 챙길 까닭이 없다.
 *
 * 어느 쪽이든 **꺼진 채로 시작한다.** 늘 켜 두면 글자만 읽고 귀로 듣지 않는다.
 *
 * **영어 영상을 고를 때 지켜야 할 것** — 사람이 직접 단 영어 자막이 있는 영상만
 * 고른다. 유튜브가 기계로 받아쓴 자막밖에 없는 영상은 **한국어로 옮길 수 없어서**
 * 한국어 자막 단추를 눌러도 영어가 그대로 나온다. 실제 브라우저에서 확인했다.
 */
export const CAPTION_LANGS = ['ko', 'en'] as const;
...
export const SCIENCE_TOPICS: ScienceTopic[] = [
  {
    ...
    // SciShow Kids 『Let's Get Rolling!』 — 무거운 것을 밀 때 바퀴가 왜 도움이
    // 되는지. 놀이터 편(JvSClZ3vHOI)이 주제에는 더 맞지만, 그 영상은 사람이 단
    // 자막이 없어 한국어 자막을 켤 수 없다(아래 참고).
    videoEn: { id: 'YIV9IUHqW3A', seconds: 199 },
  },
  {
